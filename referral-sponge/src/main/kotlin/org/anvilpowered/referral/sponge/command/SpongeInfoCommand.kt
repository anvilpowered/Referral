...
import org.anvilpowered.referral.common.command.CommonInfoCommand
...
class SpongeInfoCommand
    : CommonInfoCommand<User, Player, Text, CommandSource>(Player::class.java), CommandCallable {
    companion object {
        val DESCRIPTION: Optional<Text> = Optional.of(Text.of(CommonReferralCommandNode.INFO_DESCRIPTION))
        val USAGE: Text = Text.of(CommonReferralCommandNode.INFO_USAGE)
    }
    ...
    override fun testPermission(source: CommandSource): Boolean {
        return source.hasPermission(registry.getOrDefault(ReferralKeys.INFO_PERMISSION))
    }
